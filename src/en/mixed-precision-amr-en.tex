...
three datasets (Large, Marmousi3D, Salt).
...
\begin{itemize}
  ...
  \item All errors are bounded jointly (compression $\times$ precision
        ...
        same three datasets and three checkpointing algorithms (Revolve,
...
tested, not merely assumed, especially because AMD offers no direct
equivalent to the Ozaki scheme or to nvCOMP/Bitcomp.

\textbf{Method:} with the dedicated time that a 36-month programme
allows, we port the same tagging and tiling logic to Santos Dumont's AMD
Instinct MI300A nodes via ROCm/HIP (rocBLAS, hipBLAS, and AMD's own
reduced-precision matrix paths), without assuming feature parity with the
CUDA-specific path.

\begin{itemize}
  \item We test whether the Phase 3 criteria and the precision
        $\times$ compression factorial methodology hold on a CDNA3
        architecture, re-running the same evaluation protocol used on
        the NVIDIA GPUs.
...
The goal is to determine whether Precision-AMR is a portable methodology
or an NVIDIA-specific optimization.
