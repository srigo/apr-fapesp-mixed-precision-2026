...
\usepackage[brazilian,english]{babel} % english = main language
...
% ============================================================================
% COVER SHEET 1 (Portuguese) --- FAPESP APR, roteiro item 0:
% title, Principal Investigator, Host Institution, and a 20-line abstract.
% ============================================================================
\thispagestyle{empty}
...
% ============================================================================
% COVER SHEET 2 (English) --- FAPESP APR, roteiro item 0.
% ============================================================================
\thispagestyle{empty}
...
so numerical fidelity is essential. Closely related, Full Waveform Inversion
(FWI) estimates the subsurface velocity model itself, iteratively minimizing
the difference between observed and simulated seismograms. Its gradient is
obtained by the adjoint method, correlating the forward wavefield with the
back-propagated adjoint field. RTM and FWI therefore share the same
computational core, forward propagation followed by backward propagation
with correlation between the two fields, and consequently the same
bottleneck: both are memory-intensive, require hundreds of gigabytes of
wavefield state, and rely on checkpointing to trade memory against
recomputation in that adjoint structure. In FWI the cost is further
amplified, because this structure repeats at every inversion iteration.
...
seismic wave simulation and, more recently, explicitly for
FWI~\cite{thrastarson2020wavefield2,takekawa2016adaptive} and
RTM~\cite{yang2024elasticrtm,lu2024nested,cho2019frequency,liu2020topography}
...
coarser one. AMR addresses this by refining $\Delta x$ locally. Over four
decades, since the seminal work of Berger and Colella~\cite{berger1989amr},
this idea has consolidated into a well-established taxonomy of adaptivity
axes: cell-size refinement (h), polynomial-order enrichment (p), the
combination of the two (hp), and vertex relocation without changing the
topology (r). All of these axes, however, act on the geometric
discretization of the domain~\cite{vivarelli2025review}. No comparable,
physically motivated scheme exists for floating-point precision: existing
mixed-precision work in seismic imaging is either global and uniform (as
above), or adaptive only at the level of an individual linear-algebra
operation (mixed-precision GEMM, iterative refinement, Ozaki-scheme
emulation~\cite{ozaki2012errorfree}), not tied to a spatial decomposition of
the wavefield itself. None of this prior work varies floating-point
precision according to physically motivated spatial criteria. A literature
survey we conducted on AMR and on AMR applied to RTM/FWI confirms this
picture: the works retrieved adapt mesh resolution, order, or position, and
none adapts arithmetic precision. This is therefore an axis of adaptivity
orthogonal to the established ones, and one that remains largely
unexplored.
...
per-tile error target. It amounts to adding an arithmetic axis to a
taxonomy of adaptivity that, since Berger and Colella~\cite{berger1989amr},
has remained entirely geometric~\cite{vivarelli2025review}.
...
motivates \pamr{}'s sensitivity (FWI) criterion.

This picture is consistent with the broader state of the art in AMR. The
review by Vivarelli et al.~\cite{vivarelli2025review} identifies the
refinement indicator as the hardest open question in the field: indicators
based on local features of the field are cheap, but tend to over-refine and
do not guarantee a reduction of the error in the quantity of interest,
whereas adjoint-guided indicators achieve higher accuracy in fewer
adaptation steps, at the cost of greater complexity. On the infrastructure
side, block-structured decomposition is the granularity that sustains AMR
codes at exascale~\cite{zhang2021amrex}, and the interaction between
adaptivity and increasing hardware heterogeneity is recognized by the AMR
community itself as an additional, still unresolved axis of
complexity~\cite{dubey2021portability}.

None of these works, however, varies \emph{floating-point precision}
instead of \emph{mesh resolution}. Nor does any of them characterize
empirically how the
...
\paragraph{Target application.} All phases are implemented and validated on
Awave-3D, the RTM implementation developed in house by our group under the
guidance of geophysics experts. Awave-3D solves the acoustic wave equation
by finite differences, distributes shots across multiple GPUs, and
implements the adjoint forward and backward propagation structure with the
three checkpointing algorithms used in this project (Revolve, zCut, and
Uniform). It is the application on which GPUZIP was built and
evaluated~\cite{maltempi2025checkpointing}, which means we start from a code
base we fully control, with performance and image-quality baselines already
established on the same datasets we will use here. That familiarity is what
makes it feasible to instrument the solver at block and time-step
granularity, as Phase 2 requires.

The validation scope of this project is therefore deliberately focused:
\pamr{} will be demonstrated on RTM, on top of Awave-3D. This delimitation
is methodological, not conceptual. The four tagging criteria are formulated
in terms of quantities that any adjoint solver of hyperbolic PDEs
possesses, namely the absorbing layer, the energy density of the field, the
magnitude of the adjoint sensitivity, and the position of the wavefront,
and they depend on nothing specific to seismic imaging. This is why
\pamr{} is delivered as an independent library rather than as an internal
module of Awave-3D: the immediate natural extension is FWI, which shares
the same computational core, but the same mechanism can be applied to other
wave-propagation applications in future work.
...
There is, however, an encouraging precedent on the neighboring geometric
axis: in seismic AMR, the input velocity model is already used successfully
as an \emph{a priori} predictive signal for adapting resolution. Takekawa
and Mikada~\cite{takekawa2016adaptive} subdivide calculation points located
in low-velocity zones; Liu and Zhang~\cite{liu2020topography} use coarse
grids in high-velocity zones and fine grids in low-velocity zones for
LSRTM; Roberts et al.~\cite{roberts2022spyro} size elements by the local
P-wavespeed and the peak source frequency; Aghamiry et
al.~\cite{aghamiry2022wavelength} tune stencil weights to the local number
of grid points per wavelength; and Santos et
al.~\cite{santos2024optimaltransport} adapt the mesh directly to the
velocity field through optimal transport, delineating salt bodies and sharp
interfaces without incurring the I/O cost of re-meshing. None of these
works, however, tests whether the same signal predicts \emph{tolerance to
round-off error}, and not merely the required spatial resolution. It is
exactly this transposition, from an already validated geometric axis to the
arithmetic axis, that Phase 2 investigates.
...
\begin{itemize}
  ...
  \item \textbf{Sensitivity criterion (FWI):} tiles are tagged using the
        ...
        LeVeque~\cite{davis2015adjoint} and to the truncation-error
        estimators of classical AMR~\cite{berger1989amr}.
...
The four criteria deliberately cover the two families of indicators
identified in the AMR literature~\cite{vivarelli2025review}: the boundary,
energy, and wavefront-trailing criteria rest on local features of the field
and are cheap to evaluate, whereas the sensitivity criterion is
adjoint-guided, more expensive but targeted at the quantity of interest.
Indicators grounded in physical structure, rather than in raw gradients,
have already demonstrated cost reductions of up to 70\% in AMR for
fluid flows~\cite{kumar2025dominant}, which supports the methodological bet
of this phase.
...
component that must be rebuilt for AMD. The difficulty is not specific to
this project either: retaining performance portability in adaptive codes in
the face of increasing hardware heterogeneity is recognized as an open
problem by the AMR community itself~\cite{dubey2021portability}.
...
The scheme, introduced by Ozaki et al.~\cite{ozaki2012errorfree},
decomposes a matrix product into a sum of error-free products, which makes
it possible to recover high-precision accuracy from lower-precision
routines. Adoption: reserved for the subset of tiles that \pamr{}'s
sensitivity criteria mark as accuracy-critical (near the wavefront, high
...
\begin{itemize}
  \item \textbf{Prof.\ Dr.\ Sandro Rigo}, Principal Investigator. Associate
        Professor at the Institute of Computing, UNICAMP.
  \item \textbf{Prof.\ Dr.\ Herv\'e Yviquel}, Associated Researcher.
        Assistant Professor at the Institute of Computing, UNICAMP.
  \item \textbf{Thiago Maltempi}, collaborator. Graduate researcher (PhD
        candidate) at the Institute of Computing, UNICAMP, and lead author
        of GPUZIP~\cite{maltempi2025checkpointing}.
\end{itemize}

% TODO: the activity descriptions are to be written last. Per the APR
% roteiro, include a brief description (up to one paragraph each) of the
% activities of each team member above, plus the requested scholarship
% holders.

\emph{[Placeholder --- the description of each member's activities will be
written last, in up to one paragraph per person, including the requested
scholarship holders.]}
...
To quantify the benefit of adaptivity independently of architecture and
implementation, we adopt metrics in the spirit of those proposed by
Beisiegel et al.~\cite{beisiegel2021metrics} for AMR, correlating numerical
error, effective resolution, and run time per degree of freedom. This makes
it possible to separate the gain attributable to precision zoning itself
from the gain attributable to the implementation on a specific GPU.

\subsection{Dissemination strategy}

The project's results will be disseminated through four complementary
channels:
...
\begin{itemize}
  ...
  \item \textbf{Open-source software:} the Precision-AMR library and GPUZIP
        ...
        metrics) made available via UNICAMP's REDU.
  \item \textbf{Community uptake:} GPUZIP has already been released as
        open source and has drawn interest from research groups at UFRN,
        UFRGS, and USP in incorporating GPU-side compression into their own
        scientific applications. Precision-AMR, delivered as an independent
        library, broadens this adoption potential beyond RTM/FWI, and
        tracking these integrations is itself an indicator of the
        research's dissemination.
